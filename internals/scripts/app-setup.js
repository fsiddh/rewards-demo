...
const SUPPORTED_APP_TYPES = [NATIVE_APP_TYPE, EXTERNAL_APP_TYPE, GLOBAL_APP_TYPE];
...
const setup = async (appName, projectDir, useTDD = false) => {
  try {
    ...
    const configPath = projectDir ? `${projectDir}/${appConfigFileName}` : appConfigFileName;
    if (fs.existsSync(configPath)) {
      console.log(
        chalk.green('Configuration already exists!, You can still modify certain fields.'),
      );
    ...
    const answers = await inquirer.prompt([
      ...
      },
      {
        type: 'input',
        name: 'cluster',
        message: `Which cluster is this application created in? (${Object.keys(
          SUPPORTED_APP_CLUSTERS,
        )?.join(', ')})`,
        default: 'NIGHTLY',
      ...
      },
      {
        ...
        name: 'bugsnagApiKey',
        message:
          'Enter your Bugsnag API key (this should be a public key with push-metrics permission)',
      ...
      },
      {
        ...
        name: 'useSourceMaps',
        message:
          'Do you want to use sourcemaps? (helps in debugging but increases bundle size, recommended for UAT)',
        default: false,
        when: answers => !answers.useBugsnag,
      },
      {
      ...
      },
      {
        ...
        name: 'appNames',
        message:
          'Since this app is not hosted on Vulcan UI platform, please enter the Locize app names to be used in your app for I18N (comma separated):',
        when: answers =>
          answers.useI18n &&
      ...
      },
      {
        type: 'confirm',
        name: 'customI18n',
        message: 'Do you want custom API integration for I18N instead of Locize?',
        default: false,
      ...
      },
      {
        ...
        default: false,
        when: answers => answers.useI18n && !answers.customI18n,
      },
      {
        type: 'input',
        name: 'locales',
        message: 'Enter the locales supported for your app (comma separated):',
        when: answers => answers.customI18n || answers.localI18n,
        default: 'en-US,ja-JP',
        filter: input => input.replace(/\s/g, ''),
        validate: input => {
          ...
          const isValid = locales.every(locale => /^[a-z]{2}-[A-Z]{2}$/.test(locale));
          if (!isValid) {
      ...
      },
      {
        ...
        message:
          'Do you want to use Capillary Navigation Component in your project? (Top navbar with org switcher and other default blocks)',
        default: true,
        when: answers =>
          answers.appType !== EXTERNAL_APP_TYPE || existingConfig.appType !== EXTERNAL_APP_TYPE,
      },
    ]);
    ...
    const config = {
      ...
      appId: answers.appId,
      intouchBaseUrl:
        SUPPORTED_APP_CLUSTERS[answers.cluster?.toUpperCase()] ?? SUPPORTED_APP_CLUSTERS['NIGHTLY'],
      prefix: `/${appName}/ui`,
      ...
      i18n: {
        ...
        localI18n: answers.localI18n ?? false,
    ...
    if (config?.i18n?.localI18n && config?.i18n?.locales?.length > 0) {
      config?.i18n?.locales.forEach(locale => {
        ...
        if (!fs.existsSync(localeFileName)) {
          ...
          console.log(chalk.green(`Locale translations file created: ${localeFileName}  ✅`));
        }
      });
    }

    console.log(chalk.green('Configuration saved successfully to app-config.js!  ✅'));
    return config;
  } catch (error) {
