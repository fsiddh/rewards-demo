...
const capReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case actionTypes.GET_SIDEBAR_MENU_DATA_SUCCESS:
      return state.set('sidebarMenuData', fromJS({ status: SUCCESS, data: action.data }));
    case actionTypes.GET_SIDEBAR_MENU_DATA_FAILURE:
      return state.set('sidebarMenuData', fromJS({ status: FAILURE, error: action.error }));
    ...
    case actionTypes.GET_TOPBAR_MENU_DATA_SUCCESS:
      return state.set('topbarMenuData', fromJS({ status: SUCCESS, data: action.data }));
    case actionTypes.GET_TOPBAR_MENU_DATA_FAILURE:
      return state.set('topbarMenuData', fromJS({ status: FAILURE, error: action.error }));
    ...
    case actionTypes.SWITCH_ORG_FAILURE:
      return state.set('changeOrg', fromJS({ status: FAILURE, error: action.error }));
    ...
    case actionTypes.GET_SUPPORTED_LOCALES_FAILURE:
      return state.set('fetchingSupportedLocales', FAILURE).set('supportedLocales', fromJS([]));
    case actionTypes.GET_ORG_DETAILS_REQUEST:
      return state.set('organizationMetadata', fromJS({ status: REQUEST }));
    case actionTypes.GET_ORG_DETAILS_SUCCESS:
      return state.set('organizationMetadata', fromJS({ status: SUCCESS, data: action.data }));
    case actionTypes.GET_ORG_DETAILS_FAILURE:
      return state.set('organizationMetadata', fromJS({ status: FAILURE, error: action.error }));
    default:
      return state;
  }
};
