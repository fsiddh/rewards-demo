...
const zipDirectory = async (sourceDir, outputFilePath) => {
  if (AppConfig.isHostedOnPlatform) {
    ...
    console.log(
      chalk.green('You can now upload the build.zip file to the Vulcan UI for deployment!!'),
    );
  } else {
    console.log(
      chalk.yellow(
        'You have chosen not to host this app on Vulcan UI, so zip creation is being skipped',
      ),
    );
  }
};
