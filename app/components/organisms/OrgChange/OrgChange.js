...
const { logNewTab, utilsGetOrgNameFromId, tabBeforeUnloadEventHandler, utilsHandleStorageChange } =
  multipleOrgSwitch;
...
export const OrgChange = ({
  ...
  const handleVisibilityChange = () => {
    ...
    if (!isHiddenDoc && showRefreshModal && refreshSecondsCache === ORG_REFRESH_SEC) {
  ...
  const getOrgNameFromId = (orgId) => {
    const { user: { proxyOrgList = [], orgName: defaultOrgName, orgID: defaultOrgID } = {} } =
      userData;
    return utilsGetOrgNameFromId(orgId, proxyOrgList, defaultOrgID, defaultOrgName);
  };
    ...
    return (
      ...
      >
        <CapRow className="modal-body">
          <FormattedHTMLMessage {...messages.refreshOrgText} values={{ oldOrgName, newOrgName }} />
  ...
  const renderOrgChangeModal = () => {
    const { currentOrgDetails: { basic_details: { name: oldOrgName } = {} } = {} } = userData;
    ...
    return (
      ...
      >
        <FormattedHTMLMessage {...messages.orgChangeText} values={{ oldOrgName, newOrgName }} />
      </CapModal>
    );
  };
  ...
  useEffect(() => {
    const currentOrgName = userData?.currentOrgDetails?.basic_details?.name || null;
