    ...
    validate: () => {
      if (appConfig.bugsnag.useBugsnag) {
        if (!appConfig.bugsnag.apiKey) {
          ...
          console.log(
            chalk.green(
              'app-config.js validation: you have opted to use Bugsnag and apiKey is provided. ✅',
            ),
          );
        }
      } else {
    ...
    validate: () => {
      if (appConfig.gtm.useGTM) {
        if (!appConfig.gtm.projectId) {
          ...
          console.log(
            chalk.green(
              'app-config.js validation: you have opted to use Google Tag Manager and project Id is provided. ✅',
            ),
          );
        }
      } else {
        console.log(
          chalk.green('app-config.js validation: you have not opted to use Google Tag Manager. ✅'),
        );
      }
    },
  },
