(() => {
  ...
  console.log(
    chalk.cyan('Title tag in index.html matches with app-config?', titleMatchesWithConfig),
  );
  if (matchTitle) {
  ...
  console.log(
    chalk.cyan('Body class in index.html matches with app-config?', bodyClassMatchesWithConfig),
  );
  if (matchBodyClass) {
  ...
  console.log(
    chalk.cyan('Container ID in index.html matches with app-config?', containerIdMatchesWithConfig),
  );
  if (matchContainerId) {
    if (containerIdMatchesWithConfig) {
      ...
      indexHtml = indexHtml.replace(
        REGEX_MATCH_CONTAINER_ID,
        `id="${appConfig.appName}-container"`,
      );
      console.log(chalk.green('Container ID in index.html is now in sync with app-config ✅'));
    }
  } else {
    console.log(
      chalk.yellow('Container ID pattern in index.html not matching vulcan boilerplate!'),
    );
  }
  ...
  console.log(
    chalk.cyan(
      'Body class in global-styles.js matches with app-config?',
      bodyClassStylesMatchesWithConfig,
    ),
  );
  if (matchBodyClassStyles) {
    if (bodyClassStylesMatchesWithConfig) {
      console.log(
        chalk.green('Body class in global-styles.js is already in sync with app-config ✅'),
      );
    } else {
      console.log(chalk.yellow('Syncing the body class in global-styles.js with app-config'));
      globalStyles = globalStyles.replace(
        REGEX_MATCH_BODY_CLASS_STYLES,
        `.${appConfig.appName}-body`,
      );
      console.log(chalk.green('Body class in global-styles.js is now in sync with app-config ✅'));
    }
  } else {
    console.log(
      chalk.yellow('Body class pattern in global-styles.js not matching vulcan boilerplate!'),
    );
  }
  ...
  console.log(
    chalk.cyan(
      'Container ID in global-styles.js matches with app-config?',
      containerIdStylesMatchesWithConfig,
    ),
  );
  if (matchContainerIdStyles) {
    if (containerIdStylesMatchesWithConfig) {
      console.log(
        chalk.green('Container ID in global-styles.js is already in sync with app-config ✅'),
      );
    } else {
      console.log(chalk.yellow('Syncing the container ID in global-styles.js with app-config'));
      globalStyles = globalStyles.replace(
        REGEX_MATCH_CONTAINER_ID_STYLES,
        `#${appConfig.appName}-container`,
      );
      console.log(
        chalk.green('Container ID in global-styles.js is now in sync with app-config ✅'),
      );
    }
  } else {
    console.log(
      chalk.yellow('Container ID pattern in global-styles.js not matching vulcan boilerplate!'),
    );
  }
